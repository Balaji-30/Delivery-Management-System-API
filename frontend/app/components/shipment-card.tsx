...
export default function ShipmentCard({ shipment }: { shipment: ShipmentRead }) {
    ...
    const normalizedStatus = typeof latestStatus === "string"
        ? latestStatus.replace(/\s+/g, "_").toLowerCase()
        : latestStatus;
    const statusColor = (statusColors as Record<string, { bgColor: string; outlineColor: string }>)[normalizedStatus] ?? statusColors.placed;
    const linePositionClass = "left-[calc(15px+30px+15px+20px-1px)]";
    return (
        <Card className="shadow-none" >
            ...
            <CardContent className="grid gap-4">
                <div className="flex flex-col rounded-xl p-[15px] bg-gray-100 relative">
                    {/* Lines */}
                    <div className={`absolute top-0 bottom-0 w-0.5 bg-gray-300 ${linePositionClass}`} />
                    <div data-line className={`absolute top-6 bottom-0 w-0.5 ${statusColor.bgColor} ${linePositionClass}`} />
...
function TimelineEvent({ event, bgColor, outlineColor, hasOutline = false }: { event: ShipmentEvent, bgColor: string, outlineColor: string, hasOutline?: boolean }) {
    return (
        <div className="flex items-center gap-x-[15px]">
            <p className="text-xs text-muted-foreground w-[30px] shrink-0 text-right">
                {event.created_at.split("T")[1].slice(0, 5)}
            </p>
            <div className={`w-10 h-10 shrink-0 ${bgColor} text-foreground flex items-center justify-center rounded-full ${hasOutline ? `outline-2 ${outlineColor} outline-offset-2` : ''}`}>
                {(() => {
                    const key = typeof event.status === "string"
                        ? event.status.replace(/\s+/g, "_").toLowerCase()
                        : String(event.status);
                    return statusIcons[key] ?? <ArrowUp className="size-5 text-primary-foreground" />;
                })()}
            </div>
